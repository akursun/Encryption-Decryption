...
class ClockFactory {
    ...
    public Clock produce(String type) {
        Clock clock;
        switch(type){
            case "SAND": clock = new SandClock(); break;
            case "DIGITAL": clock = new DigitalClock(); break;
            case "MECH": clock = new MechanicalClock(); break;
            default: if(this.produceToyClock == true) {
                clock = new ToyClock();
            } else {
                return null;
            }
